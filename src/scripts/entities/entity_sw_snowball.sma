...
public plugin_precache() {
  ...
  Asset_Precache(SW_ASSET_LIBRARY, SW_ASSET_SNOWBALL_W_MODEL, g_szModel, charsmax(g_szModel));
  Asset_Precache(SW_ASSET_LIBRARY, SW_ASSET_SNOWBALL_HIT_SOUND, g_szHitSound, charsmax(g_szHitSound));
...
@Entity_Think(const this) {
  ...
  if (flAutoGuidanceRange > 0.0) {
    ...
    for (new pPlayer = 1; pPlayer <= MaxClients; ++pPlayer) {
      ...
      static Float:flDistance; flDistance = xs_vec_distance(vecOrigin, vecTargetOrigin);
...
@Entity_AutoGuidance(const this, const pTarget) {
  ...
  static Float:flAutoGuidanceAngle; flAutoGuidanceAngle = CE_GetMember(this, m_flAutoGuidanceAngle); 
