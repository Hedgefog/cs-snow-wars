...
public plugin_precache() {
  Asset_Precache(ASSET_LIBRARY, ASSET(Weapon_Fireplace_Model_View), g_szVModel, charsmax(g_szVModel));
  Asset_Precache(ASSET_LIBRARY, ASSET(Weapon_Fireplace_Model_Player), g_szPModel, charsmax(g_szPModel));
  Asset_Precache(ASSET_LIBRARY, ASSET(Weapon_Fireplace_Model_World), g_szWModel, charsmax(g_szWModel));
...
@Weapon_Create(const this) {
  ...
  CW_SetMember(this, CW_Member_iId, SW_WeaponsIds_Fireplace);
...
@Weapon_Deploy(const this) {
  ...
  CW_CallNativeMethod(this, CW_Method_DefaultDeploy, g_szVModel, g_szPModel, 1, "fireplace");
